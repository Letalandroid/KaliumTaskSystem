...
const App: React.FC = () => {
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'HIGH': 
        return <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-red-500/20 text-red-400 border border-red-500/30 glow-primary"><Zap className="w-3 h-3" /> Alta</span>;
      case 'MEDIUM': 
        return <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-amber-500/20 text-amber-400 border border-amber-500/30">Media</span>;
      case 'LOW': 
        return <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-blue-500/20 text-blue-400 border border-blue-500/30">Baja</span>;
      default: return null;
    }
  };
      ...
      <header className="glass sticky top-0 z-50 py-4 mb-2 shadow-2xl shadow-black/50">
        <div className="container mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tighter uppercase leading-none italic">
                Kalium<span className="text-primary tracking-normal not-italic ml-1">TaskSystem</span>
              </h1>
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mt-1">Core Security Protocols</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span className={`w-2 h-2 rounded-full bg-current ${kaliumMode ? 'animate-ping' : ''}`} />
          ...
          <div className="glass rounded-3xl p-8 shadow-xl sticky top-28">
            ...
            <form onSubmit={createTask} className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider ml-1">Identificador de Tarea</label>
                <input 
                  type="text" 
                  value={newTitle}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider ml-1">Nivel de Prioridad</label>
            ...
            <div className="mt-8 p-4 rounded-2xl bg-white/5 border border-white/5 flex items-start gap-3">
              ...
              <p className="text-[11px] leading-relaxed text-white/50 font-medium">
                Todas las tareas registradas bajo Modo Kalium son cifradas y priorizadas automáticamente por el núcleo del sistema.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <div className="flex items-end justify-between mb-8 px-2">
            <div>
              ...
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mt-1 ml-11">
                {kaliumMode ? 'Filtradas por Alta Prioridad' : 'Vista Global del Sistema'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 glass-pill px-4 py-2 border-white/5 shadow-lg">
              ...
              <span className="text-xs font-black tracking-tighter uppercase whitespace-nowrap">
                {tasks.filter(t => t.status === 'COMPLETED').length} / {tasks.length} <span className="text-white/30 ml-1">Completadas</span>
              </span>
            </div>
          </div>
          ...
          {loading && tasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 glass rounded-3xl opacity-50">
              ...
              <p className="text-xs font-bold uppercase tracking-[0.2em]">Accediendo al Núcleo...</p>
            </div>
          ) : tasks.length === 0 ? (
            <div className="glass rounded-[2rem] flex flex-col items-center justify-center py-32 text-center group">
              ...
              <p className="text-xl font-black italic uppercase tracking-tight mb-2">Sin amenazas pendientes</p>
              <p className="text-xs font-bold text-white/30 uppercase tracking-[0.2em]">Sistema Operativo al 100%</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {tasks.map((task) => (
                ...
                >
                  <div className={`glass rounded-2xl p-5 flex items-center gap-5 transition-all duration-500 group-hover:border-primary/40 group-hover:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] ${task.status === 'COMPLETED' ? 'bg-black/20 border-transparent' : ''}`}>
                    ...
                    >
                      <CheckCircle className={`w-5 h-5 ${task.status === 'COMPLETED' ? 'scale-100 rotate-0' : 'scale-0 rotate-90'} transition-all duration-500`} />
                      {!task.status && <div className="absolute inset-0 bg-primary/20 scale-0 group-hover:scale-100 rounded-lg transition-transform" />}
                    </button>
                    
                    <div className="flex-1 min-w-0 pr-10">
                      <h3 className={`text-lg font-black tracking-tight transition-all duration-500 truncate ${task.status === 'COMPLETED' ? 'text-white/30 line-through' : 'text-white/90 group-hover:text-white'}`}>
                        {task.title}
                      </h3>
                    ...
                    <div className={`absolute left-0 top-0 bottom-0 w-1 transition-all duration-500 ${
                      task.priority === 'HIGH' ? 'bg-red-500 shadow-[2px_0_10px_rgba(239,68,68,0.5)]' : 
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col items-center gap-4">
            <div className="flex items-center gap-2 opacity-50 grayscale hover:grayscale-0 transition-all cursor-pointer">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em]">Producido por Kalium Core</span>
            </div>
            <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">
              &copy; 2026 Ingeneria de Seguridad Kalium - Todos los derechos reservados
            </p>
          </div>
        </div>
      </footer>
